...
# @DETECTORS.register_module()
# class BEVDetSequentialESOccu(BEVDetSequentialES):
#     def __init__(self, devoxel_backbone=None,
#                  devoxel_neck=None,
#                  voxel_bev_head=None,
#                  **kwargs):
#         super(BEVDetSequentialESOccu, self).__init__(**kwargs)
...
#     def extract_feat(self, points, img, img_metas):
#         """Extract features from images and points."""
#         img_feats,voxel_feat = self.extract_img_feat(img, img_metas)
#         pts_feats = None
#         return (img_feats, pts_feats, voxel_feat)
...
#         img_feats, pts_feats, voxel_feat = self.extract_feat(
#             points, img=img_inputs, img_metas=img_metas)
#         assert self.with_pts_bbox
#         losses = dict()
#         losses_pts = self.forward_pts_train(img_feats, gt_bboxes_3d,
...
#         """Test function without augmentaiton."""
#         img_feats, _,  voxel_feat = self.extract_feat(points, img=img, img_metas=img_metas)
#         bbox_list = [dict() for _ in range(len(img_metas))]
#         bbox_pts = self.simple_test_pts(img_feats, img_metas, rescale=rescale)
#         for result_dict, pts_bbox in zip(bbox_list, bbox_pts):
#             result_dict['pts_bbox'] = pts_bbox
...
# @DETECTORS.register_module()
# class BEVDepth4D_OccuHead(BEVDepth4D):
#     def __init__(self,
#                  voxel_bev_head=None,
#                  **kwargs):
#         super(BEVDepth4D_OccuHead, self).__init__(**kwargs)
...
#             result_dict['pts_bbox'] = pts_bbox
#         if return_vox_results:
#             voxel_out = self.voxel_bev_head(img_feats[0])
#             return bbox_list, voxel_out, gt_voxel_bev
#         return bbox_list
